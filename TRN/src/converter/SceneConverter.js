...
TRN.SceneConverter.prototype = {
	...
	createMoveableAsMultiMeshes : function (itemIndex, roomIndex, x, y, z, lighting, rotation, moveable, jsonid, visible) {

		if (typeof(jsonid) == 'undefined') jsonid = 'mvoeable' + moveable.objectID + '_' + itemIndex;
        ...
        var moveableForVisu = this.trlevel.moveables[this.movObjID2Index[objIDForVisu]];
		var numMeshes = moveableForVisu.numMeshes, meshIndex = moveableForVisu.startingMesh, meshTree = moveableForVisu.meshTree;

        var stackIdx = 0, stack = [], parent = -1;
        var px = 0, py = 0, pz = 0, bones = [];

        var startId = 'moveable' + moveable.objectID;
		...
		for (var idx = 0; idx < numMeshes; ++idx, meshIndex++) {
            ...
            var tiles2material = {};
            var skinIndices = [], skinWeights = [];

            var mesh = this.trlevel.meshes[meshIndex];

			var internalLit = this.makeMeshGeometry(mesh, meshIndex, meshJSON, tiles2material, this.trlevel.objectTextures, this.trlevel.mapObjTexture2AnimTexture, 0, attributes, idx, skinIndices, skinWeights);
		...
		for (var idx = 0; idx < numMeshes; ++idx) {

			var mid = startId + '_mesh' + idx;

			var moveableIsInternallyLit = this.sc.embeds[mid].moveableIsInternallyLit;
			var materials = [];
			...
			var obj = {
                ...
                "has_anims"				: idx == 0,
                "roomIndex"				: roomIndex,
                "animationStartIndex"	: idx == 0 ? moveable.animation : -1,
                "skin"					: true,
                "use_vertex_texture" 	: false,
                "hasScrollAnim"			: this.sc.embeds[mid].objHasScrollAnim,
                "parent_for_skin"       : idx == 0 ? "" : jsonid,
                "children"              : {}
            };
        ...
        for (var idx = 0; idx < numMeshes; ++idx) {
            var obj = meshList[idx];
            var oparent = bones[idx].parent == -1 ? null : meshList[bones[idx].parent];
            if (oparent == null) {
                obj.position = [ x*TRN.Consts.worldScale, y*TRN.Consts.worldScale, z*TRN.Consts.worldScale ];
                obj.quaternion = rotation;
                this.sc.objects[jsonid] = obj;
            } else {
                oparent.children[jsonid + '_' + idx] = obj;
            }
        }
		...
		for (var i = 0; i < this.trlevel.items.length; ++i) {
			...
			if (m == null) {
				...
				if (item.objectID == this.laraObjectID) {
                    mvb = this.createMoveableAsMultiMeshes(i, roomIndex, item.x, -item.y, -item.z, lighting, q, this.trlevel.moveables[m]);
                } else {
                    mvb = this.createMoveableInstance(i, roomIndex, item.x, -item.y, -item.z, lighting, q, this.trlevel.moveables[m]);
                }
				if (item.objectID == this.laraObjectID) {
					laraMoveable = mvb;
				}
